...
class ResNet(nn.Module) :
    def __init__(self, block, num_blocks, num_classes=1) :
        ...
        self.fc2 = nn.Sequential(
            nn.Linear(1000, 100), nn.ReLU(inplace=True),
        )
        self.mlp1 = nn.Sequential(nn.Linear(8, 300), nn.BatchNorm1d(300), nn.ReLU(True))
        self.mlp2 = nn.Sequential(nn.Linear(300, 100), nn.BatchNorm1d(100), nn.ReLU(True))
    ...
    def forward(self, x, u):
        ...
        out = self.fc1(out)
        out = self.fc2(out)
        out_state = self.mlp1(u)
        out_state = self.mlp2(out_state)
        out_concat = torch.cat([out, out_state], 1)
        out = self.fc3(out_concat)
        return out.view(-1)
...
def main() :
    batchsize = 32
    train_data = myDataLoader.State_Pic(txt='data/train_pic_state.txt', transform=transforms.ToTensor())
    ...
    for epoch in range(300) :
        ...
        for i, data in enumerate(train_data_load, 0) :
            # print (inputs,labels)
            image, state, label = data
            if torch.cuda.is_available() :
                # print('GPU2')
                image = image.double().cuda()
                state = state.double().cuda()
            ...
            if i % 100 == 99 :
                ...
                running_loss = 0
                if epoch % 100 == 99:
                    torch.save(net, "net/epoch%d"%(epoch+1)+'resnet_mlp.pkl')
                    print('model has saved')
    print("finish training")
    torch.save(net, 'net/resnet_mlp.pkl')  # 保存整个网络

def test_net():
    batchsize = 1
    start = time.time
    test_data = myDataLoader.State_Pic(txt='data/test_pic_state.txt', transform=transforms.ToTensor())
    ...
    if torch.cuda.is_available() :
        ...
        net = torch.load('net/epoch400resnet_mlp.pkl').cuda()
    else:
        # net = torch.load('epoch299resnet.pkl')
        net = torch.load('net/epoch400resnet_mlp.pkl')
    net.eval()
    running_loss = 0
    for i, data in enumerate(test_data_load, 0) :
        ...
        if torch.isnan(loss):
            break
        running_loss += loss.data
    print("total test number:", len(test_data))
